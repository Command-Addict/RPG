...
mc_path = 'rpg:level/system/ultimate/thunder/damage'
...
def build_quatern(start, end, scale, now, mc_now):
    ...
    for r in split:
        if r[0] == r[1]:
            fd.write('execute if score .value rpg_virtual_data matches %d run damage @s %f minecraft:lightning_bolt by @e[tag=this,limit=1]\n'%(r[0],r[0]*scale))
        else:
